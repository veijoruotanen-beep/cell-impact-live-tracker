...
def recompute_trade_index(data):
    ...
    try:
        ...
        live = None
    data = ensure_root_scaffold(data)
...
def build_what_matters_now(data):
    ...
    latest_val = monthly.get("latest_value")
    if latest_val not in (None, ""):
        items.append({
            "label": "Monthly trade pulse",
            "value": f"{latest_val:,.1f}" if isinstance(latest_val, (int, float)) else str(latest_val),
            "note": monthly.get("note", "Monthly balanced bundle computed from COMEXT bulk files."),
        })

    macro = trade_signals.get("macro_signal", {}) or {}
    if macro.get("value") not in (None, ""):
        items.append({
            "label": "Macro backdrop",
            "value": str(macro.get("value")),
            "note": macro.get("note", "Annual macro/proxy backdrop."),
        })

    if customs.get("latest_index") not in (None, ""):
        items.append({
            "label": "Customs proxy",
            "value": str(customs.get("latest_index")),
            "note": customs.get("coverage_note", "Customs proxy signal."),
        })
