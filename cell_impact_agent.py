...
def choose_latest_archive(rows, debug):
    ...
    debug["archive_candidates"] = [n for _, n in candidates[:50]]
    if not candidates:
        raise RuntimeError("No matching full*_YYYYMM.7z files found in COMEXT_DATA/PRODUCTS listing")
    yyyymm, name = sorted(candidates, key=lambda x: x[0])[-1]
    return name, yyyymm


def download_archive_bytes(name):
    file_param = urllib.parse.quote(f"comext/COMEXT_DATA/PRODUCTS/{name}", safe="")
    return fetch_bytes(FILES_API_DOWNLOAD.format(file=file_param), timeout=120)
...
def extract_first_usable_payload(blob, workdir: Path, debug):
    ...
    with io.BytesIO(blob) as bio:
        with py7zr.SevenZipFile(bio, mode='r') as z:
            names = z.getnames()
            debug["archive_members"] = names[:500]
            z.extractall(path=first_pass_dir)

    all_files = [p for p in first_pass_dir.rglob("*") if p.is_file()]
    debug["extracted_all_files"] = [str(p.relative_to(workdir)) for p in all_files[:500]]

    found_nested_7z = []
    for p in sorted(all_files, key=lambda p: len(str(p))):
        lname = p.name.lower()
        ...
        if lname.endswith(".7z"):
            found_nested_7z.append(p)

    if found_nested_7z:
        nested = found_nested_7z[0]
        second_dir = workdir / "second_pass"
        second_dir.mkdir(parents=True, exist_ok=True)
        with py7zr.SevenZipFile(nested, mode='r') as z:
            nested_names = z.getnames()
            debug["nested_archive"] = str(nested.relative_to(workdir))
            debug["nested_members"] = nested_names[:500]
            z.extractall(path=second_dir)
        second_files = [p for p in second_dir.rglob("*") if p.is_file()]
        debug["second_pass_files"] = [str(p.relative_to(workdir)) for p in second_files[:500]]
        for p in sorted(second_files, key=lambda p: len(str(p))):
            lname = p.name.lower()
            if lname.endswith((".csv", ".tsv", ".txt", ".dat", ".parquet")):
                debug["extracted_file"] = str(p.relative_to(workdir))
                debug["extracted_kind"] = p.suffix.lower()
...
def read_tabular(path: Path, debug):
    ...
    debug["delimiter_guess"] = delimiter
...
def map_columns(columns):
    return {
        ...
        "product": choose_column(columns, ["product", "prod", "commodity", "cn", "hs"]),
        "value": choose_column(columns, ["value", "obs_value", "trade_value", "valeur", "obsValue"]),
    }
...
def normalize_partner(v):
    if v is None:
        return None
    s = str(v).upper().strip()
    aliases = {"USA": "US", "UNITED STATES": "US", "CHN": "CN", "JPN": "JP", "KOR": "KR"}
...
def aggregate_monthly(rows, cmap, debug):
    ...
    required = ["period", "flow", "partner", "product", "value"]
    ...
    for row in rows:
        ...
        if not period or flow not in {"EXP", "IMP"} or partner not in PARTNERS or prod4 not in HS4_CODES or val is None:
            continue
        if geo_col and geo and "EU27_2020" not in geo and "EU27" not in geo:
            continue
...
def recompute_monthly_bundle(data):
    ...
    try:
        ...
        yoy = None
        target_period = latest_ma["period"]
        y, m = target_period.split("-")
        prev_yoy_period = f"{int(y)-1:04d}-{m}"
        prev_yoy = latest_raw_map.get(prev_yoy_period)
        if prev_yoy not in (None, 0):
            yoy = round(((latest_raw - prev_yoy) / prev_yoy) * 100, 1)
        ...
        data["monthly_trade_pulse"].update({
            ...
            "yoy_pct": yoy,
            "note": "Monthly balanced bundle computed from COMEXT bulk file.",
        ...
        data["trade_signals"]["balanced_signal"].update({
            "status": "Live",
            "note": "Primary monthly trade pulse from COMEXT bulk file."
        })
        ...
        data["trade_signals"]["balanced_signal"]["status"] = "Fetch failed"
        data["trade_signals"]["balanced_signal"]["note"] = msg
        data["trade_signals"]["exports_signal"]["status"] = "Fetch failed"
        data["trade_signals"]["exports_signal"]["note"] = msg
        data["trade_signals"]["imports_signal"]["status"] = "Fetch failed"
        data["trade_signals"]["imports_signal"]["note"] = msg
