...
def choose_recent_archives(rows, debug, n=ARCHIVES_TO_LOAD):
    ...
    debug["archive_candidates"] = [n for _, n in candidates[:120]]
    if not candidates:
...
def aggregate_monthly(rows, cmap, debug):
    ...
    for row in rows:
        ...
        prod_raw_full = str(row.get(cmap["product"], "")).strip()
        if prod_raw_full and len(sample_products) < 20:
            sample_products.append(prod_raw_full)
        prod4 = re.sub(r"\D", "", prod_raw_full)[:4]
        if prod4:
        ...
        if len(kept_examples) < 10:
            kept_examples.append({
                "period": period, "flow": flow, "partner": partner, "prod4": prod4, "geo": geo, "value": val
            })
    ...
    debug["sample_partners"] = sample_partners
    debug["sample_geos"] = sample_geos
    debug["sample_products"] = sample_products
    debug["sample_flows"] = sample_flows
    debug["top_partners"] = top_partners.most_common(20)
    debug["top_products"] = top_products.most_common(20)
    debug["top_geos"] = top_geos.most_common(20)
    debug["top_flows"] = top_flows.most_common(20)
...
def recompute_monthly_bundle(data):
    ...
    try:
        ...
        for yyyymm, archive_name in selected:
            ...
            archive_debug["download_url"] = FILES_API_DOWNLOAD.format(
                file=urllib.parse.quote(f"comext/COMEXT_DATA/PRODUCTS/{archive_name}", safe="")
            )
            ...
            payload_path = extract_first_usable_payload(blob, workdir, archive_debug)
            part_rows = read_tabular(payload_path, archive_debug)
            all_rows.extend(part_rows)
            processed_archives.append(archive_debug)
            archive_row_counts.append({"archive": archive_name, "rows": len(part_rows)})

        debug["processed_archives"] = processed_archives[-6:]  # keep debug manageable
        debug["archive_row_counts"] = archive_row_counts

        if not all_rows:
            raise RuntimeError("No rows collected from selected COMEXT monthly archives")

        cmap = map_columns(processed_archives[-1].get("columns", []))
        exports_series, imports_series, balanced_raw, balanced_ma = aggregate_monthly(all_rows, cmap, debug)
        ...
        data["monthly_trade_pulse"].update({
            ...
            "balanced_series": balanced_ma[-WINDOW_MONTHS:],
            "balanced_raw_series": balanced_raw[-WINDOW_MONTHS:],
            "exports_series": exports_series[-WINDOW_MONTHS:],
            "imports_series": imports_series[-WINDOW_MONTHS:]
        })
        ...
        data["trade_signals"]["exports_signal"].update({
            "status": "Live",
            "note": "Exports slice from monthly COMEXT bundle."
        })
        data["trade_signals"]["imports_signal"].update({
            "status": "Live",
            "note": "Imports slice from monthly COMEXT bundle."
        })
