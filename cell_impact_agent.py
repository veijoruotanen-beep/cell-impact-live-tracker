...
def pick_latest_archive(rows):
    candidates = []
    pattern = re.compile(r"^full.*?_(\d{6})\.7z$", re.I)
    fallback = re.compile(r"^full.*?(\d{6})\.7z$", re.I)
    for row in rows:
        name = row.get("name") or row.get("file") or ""
        m = pattern.match(name) or fallback.match(name)
        if m:
    ...
    return latest[1], [name for _, name in sorted(candidates)[-20:]]
...
def monthly_from_bulk():
    debug = {}
    listing_url, rows = files_api_listing()
