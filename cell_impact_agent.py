...
def extract_data_file_from_7z(archive_bytes):
    if py7zr is None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        rels = [os.path.relpath(p, td) for p in files_found]
        text_like = [p for p in files_found if p.lower().endswith((".csv", ".tsv", ".txt"))]
        parquet_like = [p for p in files_found if p.lower().endswith((".parquet", ".pq"))]
        nested_archives = [p for p in files_found if p.lower().endswith((".zip", ".7z"))]
        # try nested archives once
        if not text_like and nested_archives:
            nested = nested_archives[0]
            if nested.lower().endswith('.7z'):
                with py7zr.SevenZipFile(nested, mode='r') as z2:
                    z2.extractall(path=os.path.join(td,'nested'))
            # zip support intentionally omitted; just expose in debug if needed
            for root, _, files in os.walk(os.path.join(td,'nested')):
                for file in files:
                    full = os.path.join(root, file)
                    if full.lower().endswith((".csv", ".tsv", ".txt")):
                        text_like.append(full)
                    if full.lower().endswith((".parquet", ".pq")):
                        parquet_like.append(full)
                    rels.append(os.path.relpath(full, td))
        if text_like:
            chosen = sorted(text_like)[0]
            ...
            return {
                ...
                'archive_members': rels[:200]
            }
        if parquet_like:
            return {
                'path': sorted(parquet_like)[0],
                'kind': 'parquet',
                'sample': '',
                'archive_members': rels[:200]
            }
        raise RuntimeError("No CSV/TSV/TXT/PARQUET file found inside COMEXT archive")
...
def monthly_from_bulk():
    ...
    extracted = extract_data_file_from_7z(archive_bytes)
    csv_path = extracted['path']
    sample_text = extracted.get('sample','')
    debug['extracted_file'] = os.path.basename(csv_path)
    debug['archive_members'] = extracted.get('archive_members', [])
    debug['extracted_kind'] = extracted.get('kind')
    if extracted.get('kind') == 'parquet':
        raise RuntimeError(f"Archive contains parquet, not text. File: {os.path.basename(csv_path)}")
