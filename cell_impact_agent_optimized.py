...
ARCHIVE_CANDIDATE_DEBUG_LIMIT = 40
TOP_DEBUG_LIMIT = 12
PROCESSED_ARCHIVE_DEBUG_LIMIT = 4
...
def choose_recent_archives(rows, debug, n=ARCHIVES_TO_LOAD):
    ...
    debug["archive_candidates"] = [name for _, name in candidates[:ARCHIVE_CANDIDATE_DEBUG_LIMIT]]
    if not candidates:
...
def extract_first_payload(blob, workdir: Path, debug):
    ...
    with io.BytesIO(blob) as bio:
        with py7zr.SevenZipFile(bio, mode="r") as z:
            debug["archive_members"] = z.getnames()[:80]
            z.extractall(path=p1)
    files = [p for p in sorted(p1.rglob("*")) if p.is_file()]
    debug["extracted_all_files"] = [str(p.relative_to(workdir)) for p in files[:80]]
    for p in files:
        lname = p.name.lower()
