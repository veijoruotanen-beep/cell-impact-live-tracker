...
def recent_period_keys(target_period, periods, lookback=3):
    ordered = sorted(str(p) for p in periods if p)
    if target_period not in ordered:
        return []
    pos = ordered.index(target_period)
    start = max(0, pos - lookback)
    return ordered[start:pos]


def summarize_breadth(snapshot):
    improving = int(snapshot.get("improving", 0) or 0)
    total = int(snapshot.get("total", 0) or 0)
    pct = snapshot.get("pct")
    headline = f"{pct:.0f}%" if isinstance(pct, (int, float)) else "n/a"
    status = snapshot.get("status") or "n/a"
    improving_labels = [item.get("label") for item in snapshot.get("improving_slices", []) if item.get("label")]
    weak_labels = [item.get("label") for item in snapshot.get("weakening_slices", []) if item.get("label")]
    improving_txt = ", ".join(improving_labels[:3]) if improving_labels else "none"
    weak_txt = ", ".join(weak_labels[:3]) if weak_labels else "none"
    note = f"{improving}/{total} partner/flow slices improving vs prior 3M average. Improving: {improving_txt}. Weakening: {weak_txt}." if total else "No eligible slices yet."
    return {"headline": headline, "status": status, "note": note, "improving": improving, "total": total, "pct": pct}


def compute_breadth_from_maps(periods, partner_flow_map, product_map, geo_map, latest_period):
    def classify(pct):
        if pct is None:
            return "n/a"
        if pct >= 70:
    ...
    def calc(map_obj, labels):
        prev_periods = recent_period_keys(latest_period, periods, 3)
        improving, weakening = [], []
        for key in labels:
            history = map_obj.get(key, {}) or {}
            latest_val = history.get(latest_period)
            prev_vals = [history.get(p) for p in prev_periods]
            prev_vals = [float(v) for v in prev_vals if isinstance(v, (int, float))]
            if not isinstance(latest_val, (int, float)) or len(prev_vals) < 2:
                continue
            baseline = sum(prev_vals) / len(prev_vals)
            if baseline <= 0:
                continue
            change_pct = ((float(latest_val) - baseline) / baseline) * 100.0
            item = {"label": key, "latest": round(float(latest_val), 2), "baseline": round(float(baseline), 2), "change_pct": round(change_pct, 1)}
            (improving if change_pct > 0 else weakening).append(item)
        total = len(improving) + len(weakening)
        pct = round((len(improving) / total) * 100.0, 1) if total else None
        return {
            "latest_period": latest_period,
            "comparison_basis": "vs prior 3M average",
            "improving": len(improving),
            "total": total,
            "pct": pct,
            "status": classify(pct),
            "improving_slices": sorted(improving, key=lambda x: x["change_pct"], reverse=True)[:6],
            "weakening_slices": sorted(weakening, key=lambda x: x["change_pct"])[:6],
        }

    partner_labels = [f"{partner}_{flow}" for partner in sorted(PARTNERS) for flow in ("EXP", "IMP")]
    product_labels = sorted(HS4_CODES)
    geo_labels = sorted((g for g in geo_map.keys() if g))[:20]

    partner_snapshot = calc(partner_flow_map, partner_labels)
    product_snapshot = calc(product_map, product_labels)
    geo_snapshot = calc(geo_map, geo_labels)

    total_improving = sum(s.get("improving", 0) for s in (partner_snapshot, product_snapshot, geo_snapshot))
    total_total = sum(s.get("total", 0) for s in (partner_snapshot, product_snapshot, geo_snapshot))
    total_pct = round((total_improving / total_total) * 100.0, 1) if total_total else None

    return {
        "latest_period": latest_period,
        "comparison_basis": "vs prior 3M average",
        "overall_pct": total_pct,
        "overall_status": classify(total_pct),
        "partner_flow": partner_snapshot,
        "product": product_snapshot,
        "geo": geo_snapshot,
        "headline": summarize_breadth(partner_snapshot),
    }
...
def process_archive_rows(row_iter, cmap, debug):
    ...
    kept_examples = []
    exp, imp = {}, {}
    partner_flow_map, product_map, geo_map = {}, {}, {}
    ...
    for row in row_iter:
        ...
        if geo_col and geo and geo not in EU_REPORTERS:
            ...
            continue
        rows_kept += 1
        partner_flow_map.setdefault(f"{partner}_{flow}", {})[period] = partner_flow_map.setdefault(f"{partner}_{flow}", {}).get(period, 0.0) + val
        product_map.setdefault(prod4, {})[period] = product_map.setdefault(prod4, {}).get(period, 0.0) + val
        if geo:
            geo_map.setdefault(geo, {})[period] = geo_map.setdefault(geo, {}).get(period, 0.0) + val
    ...
    return exp, imp, partner_flow_map, product_map, geo_map
...
def ensure_monthly_scaffold(data):
    data["trade_signals"] = {
        ...
        "breadth_signal": {"status": "Pending live run", "scope": "Partner / product / geo participation", "value": None, "meta": "Share of tracked slices improving vs prior 3M average.", "note": "Breadth will update after the agent computes slice participation from COMEXT monthly extraction."},
...
def recompute_monthly_bundle(data):
    ...
    try:
        ...
        exp_total, imp_total = {}, {}
        partner_flow_totals, product_totals, geo_totals = {}, {}, {}
        ...
        for yyyymm, archive_name in selected:
            ...
            with open(payload_path, "r", encoding="utf-8", errors="ignore", newline="") as f:
                ...
                exp_part, imp_part, partner_flow_part, product_part, geo_part = process_archive_rows(reader, cmap, archive_debug)
            ...
            for key, period_map in partner_flow_part.items():
                dest = partner_flow_totals.setdefault(key, {})
                for period_key, val in period_map.items():
                    dest[period_key] = dest.get(period_key, 0.0) + val
            for key, period_map in product_part.items():
                dest = product_totals.setdefault(key, {})
                for period_key, val in period_map.items():
                    dest[period_key] = dest.get(period_key, 0.0) + val
            for key, period_map in geo_part.items():
                dest = geo_totals.setdefault(key, {})
                for period_key, val in period_map.items():
                    dest[period_key] = dest.get(period_key, 0.0) + val
        ...
        breadth = compute_breadth_from_maps(periods, partner_flow_totals, product_totals, geo_totals, latest_ma["period"])
        debug["breadth"] = breadth
        ...
        data["trade_signals"]["breadth_signal"].update({
            "status": breadth.get("overall_status") or "Live",
            "value": breadth.get("overall_pct"),
            "meta": f"Partner flow {breadth.get('partner_flow', {}).get('pct', 'n/a')}% • Product {breadth.get('product', {}).get('pct', 'n/a')}% • Geo {breadth.get('geo', {}).get('pct', 'n/a')}%",
            "note": (breadth.get("headline") or {}).get("note") or "Share of tracked slices improving vs prior 3M average."
        })

    except Exception as e:
        msg = f"Monthly COMEXT bulk extraction did not yield a usable series yet. {type(e).__name__}: {e}"
        for k in ("balanced_signal", "exports_signal", "imports_signal", "breadth_signal"):
...
def build_what_matters_now(data):
    ...
    breadth = (monthly.get("breadth") or {}).get("overall_pct")
    breadth_status = (monthly.get("breadth") or {}).get("overall_status")
    breadth_note = ((monthly.get("breadth") or {}).get("headline") or {}).get("note")
    if breadth not in (None, "") and len(items) < 4:
        items.append({
            "label": "Trade breadth",
            "value": f"{breadth:.0f}%" if isinstance(breadth, (int, float)) else str(breadth),
            "note": breadth_note or f"Participation signal across partner, product and geo slices. Status: {breadth_status or 'n/a'}."
        })
